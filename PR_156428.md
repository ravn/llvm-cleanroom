...
`+55 / -0` in `LiveVariables.cpp` — most of which is explanatory comments; the actual logic change is about a dozen lines — plus a new MIR lit test on AVR with four cases: the bug's reduction, the same shape with an intervening unrelated instruction, the partial-def-of-subreg case the 2026 commit was originally handling, and a safety guard case verifying the fix does not add an implicit-use for a sub-register with no prior def in the block.
...
> **Attribution boundary.** Everything below this line was drafted by Claude (Anthropic; models `claude-opus-4-7` for the original fix and analysis, `claude-sonnet-4-6` for the fourth test case and session of 2026-06-23) at my direction. I have reviewed it and confirm it matches the actual patch, but I did not rewrite the wording.
> — *Thorbjørn*
